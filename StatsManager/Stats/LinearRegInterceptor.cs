...
{
    public class LinearRegInterceptor : Stat
    {
        public Dictionary<Tuple<string, DateOnly>, double> LINEARREG_INTERCEPTOR = new Dictionary<Tuple<string, DateOnly>, double>();
        public LinearRegInterceptor(string symbol) : base(symbol) { OptInPeriod = 20; } //Default value
        public LinearRegInterceptor(string symbol, DateOnly from, DateOnly to, int optInPeriod, PriceType price) : base(symbol, from, to, optInPeriod, price) { }
        public void Process()
        {
            Calculate();
            IdentifyPatterns();
        ...
        public void Calculate()
        {
            ...
            Core.LinearRegIntercept(begIdx, endIdx, inReal.ToArray(), OptInPeriod, out outBegIdx, out outNBElement, outReal);

            for (int i = 0; i < outNBElement; i++)
            {
                //Console.WriteLine( "DEBUG: LINEARREG-INTERCEPT {0} {1} {2}", Symbol, DateOnly.FromDateTime(equities[outBegIdx + i].DATE), outReal[i]);
                LINEARREG_INTERCEPTOR.Add(new Tuple<string, DateOnly>(Symbol, DateOnly.FromDateTime(equities[outBegIdx + i].DATE)), outReal[i]);
            }
        }
        private void IdentifyPatterns()
        {
            ...
            foreach (KeyValuePair<Tuple<string, DateOnly>, double> item in LINEARREG_INTERCEPTOR)
            {
        ...
        {
            summary.Indicator = IndicatorType.LINEARREG_INTERCEPTOR;
            base.UpdateSummary();
        }
    }
}
