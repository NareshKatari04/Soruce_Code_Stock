...
{
    public class CdlTriStar : Stat
    {
        //Static data
        public static Dictionary<Tuple<string, DateOnly>, double> TRI_STAR = new Dictionary<Tuple<string, DateOnly>, double>();

        //Constructors
        public CdlTriStar(string symbol) : base(symbol)
        {
            summary.Indicator = IndicatorType.CDL_TRI_STAR;
            OptInPeriod = 4;
            TypeOfPrice = PriceType.ClosePrice;
        }
        public CdlTriStar(string symbol, DateOnly from, DateOnly to, int optInPeriod, PriceType price) : base(symbol, from, to, optInPeriod, price)
        {
            summary.Indicator = IndicatorType.CDL_TRI_STAR;
        }
        ...
        private void Calculate()
        {
            ...
            Core.CdlTristar(begIdx, endIdx, inRealOpen.ToArray(), inRealHigh.ToArray(), inRealLow.ToArray(), inRealClose.ToArray(), out outBegIdx, out outNBElement, outInt);

            for (int i = 0; i < outNBElement; i++)
            {
                if (outInt[i] == 100 || outInt[i] == -100)
                    TRI_STAR.Add(new Tuple<string, DateOnly>(Symbol, DateOnly.FromDateTime(equities[outBegIdx + i].DATE)), outInt[i]);
            }
        }
        private void IdentifyPatterns()
        {
            //1. For each Rsi captured
            foreach (var item in TRI_STAR)
            {
