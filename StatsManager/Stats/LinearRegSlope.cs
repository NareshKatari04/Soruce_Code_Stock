...
{
    public class LinearRegSlope : Stat
    {
        public Dictionary<Tuple<string, DateOnly>, double> LINEARREG_SLOPE = new Dictionary<Tuple<string, DateOnly>, double>();
        public LinearRegSlope(string symbol) : base(symbol) { OptInPeriod = 20; } //Default value
        public LinearRegSlope(string symbol, DateOnly from, DateOnly to, int optInPeriod, PriceType price) : base(symbol, from, to, optInPeriod, price) { }
        public void Process()
        {
            Calculate();
            IdentifyPatterns();
        ...
        public void Calculate()
        {
            ...
            Core.LinearRegSlope(begIdx, endIdx, inReal.ToArray(), OptInPeriod, out outBegIdx, out outNBElement, outReal);

            for (int i = 0; i < outNBElement; i++)
            {
                //Console.WriteLine( "DEBUG: EMA {0} {1} {2}", Symbol, DateOnly.FromDateTime(equities[outBegIdx + i].DATE), outReal[i]);
                LINEARREG_SLOPE.Add(new Tuple<string, DateOnly>(Symbol, DateOnly.FromDateTime(equities[outBegIdx + i].DATE)), outReal[i]);
            }
        }
        private void IdentifyPatterns()
        {
            ...
            foreach (KeyValuePair<Tuple<string, DateOnly>, double> item in LINEARREG_SLOPE)
            {
        ...
        {
            summary.Indicator = IndicatorType.LINEARREG_SLOPE;
            base.UpdateSummary();
        }
    }
}
