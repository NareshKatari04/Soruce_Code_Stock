...
{
    public class Dema : Stat
    {
        public Dictionary<Tuple<string, DateOnly>, double> DEMA = new Dictionary<Tuple<string, DateOnly>, double>();
        public Dema(string symbol) : base(symbol) { summary.Indicator = IndicatorType.DEMA; ;  OptInPeriod = 20; } //Default value
        public Dema(string symbol, DateOnly from, DateOnly to, int optInPeriod, PriceType price) : base(symbol, from, to, optInPeriod, price) { summary.Indicator = IndicatorType.DEMA; }
        public void Process()
        {
            Calculate();
            IdentifyPatterns();
        ...
        public void Calculate()
        {
            ...
            Core.Dema(begIdx, endIdx, inReal.ToArray(), OptInPeriod, out outBegIdx, out outNBElement, outReal);

            for (int i = 0; i < outNBElement; i++)
            {
                //Console.WriteLine( "DEBUG: DEMA {0} {1} {2}", Symbol, DateOnly.FromDateTime(equities[outBegIdx + i].DATE), outReal[i]);
                DEMA.Add(new Tuple<string, DateOnly>(Symbol, DateOnly.FromDateTime(equities[outBegIdx + i].DATE)), outReal[i]);
            }
        }
        private void IdentifyPatterns()
        {
            ...
            foreach (KeyValuePair<Tuple<string, DateOnly>, double> item in DEMA)
            {
