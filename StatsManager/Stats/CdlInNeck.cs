...
{
    public class CdlInNeck : Stat
    {
        //Static data
        public static Dictionary<Tuple<string, DateOnly>, double> IN_NECK = new Dictionary<Tuple<string, DateOnly>, double>();

        //Constructors
        public CdlInNeck(string symbol) : base(symbol)
        {
            summary.Indicator = IndicatorType.CDL_IN_NECK;
            OptInPeriod = 4;
            TypeOfPrice = PriceType.ClosePrice;
        }
        public CdlInNeck(string symbol, DateOnly from, DateOnly to, int optInPeriod, PriceType price) : base(symbol, from, to, optInPeriod, price) 
        {
            summary.Indicator = IndicatorType.CDL_IN_NECK;
        }
        ...
        private void Calculate()
        {
            ...
            for (int i = 0; i < outNBElement; i++)
            {
                if (outInt[i] == 100 || outInt[i] == -100)
                    IN_NECK.Add(new Tuple<string, DateOnly>(Symbol, DateOnly.FromDateTime(equities[outBegIdx + i].DATE)), outInt[i]);
            }
        }
        private void IdentifyPatterns()
        {
            //1. For each Rsi captured
            foreach (var item in IN_NECK)
            {
