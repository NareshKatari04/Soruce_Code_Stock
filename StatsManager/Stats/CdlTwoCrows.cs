...
{
    public class CdlTwoCrows : Stat
    {
        //Static data
        public static Dictionary<Tuple<string, DateOnly>, double> TWO_CROWS = new Dictionary<Tuple<string, DateOnly>, double>();

        //Constructors
        public CdlTwoCrows(string symbol) : base(symbol) 
        {
            summary.Indicator = IndicatorType.CDL_TWO_CROWS;
            OptInPeriod = 14;
            TypeOfPrice = PriceType.ClosePrice;
        }
        public CdlTwoCrows(string symbol, DateOnly from, DateOnly to, int optInPeriod, PriceType price) : base(symbol, from, to, optInPeriod, price) 
        {
            summary.Indicator = IndicatorType.CDL_TWO_CROWS;
        }
        ...
        private void Calculate()
        {
            ...
            for (int i = 0; i < outNBElement; i++)
            {
                if (outInt[i] == 100 || outInt[i] == -100)
                    TWO_CROWS.Add(new Tuple<string, DateOnly>(Symbol, DateOnly.FromDateTime(equities[outBegIdx + i].DATE)), outInt[i]);
            }
        }
        private void IdentifyPatterns()
        {
            //1. For each Rsi captured
            foreach (var item in TWO_CROWS)
            {
