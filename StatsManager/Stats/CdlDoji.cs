...
{
    public class CdlDoji : Stat
    {
        //Static data
        public static Dictionary<Tuple<string, DateOnly>, double> DOJI = new Dictionary<Tuple<string, DateOnly>, double>();

        //Constructors
        public CdlDoji(string symbol) : base(symbol)
        {
            summary.Indicator = IndicatorType.CDL_DOJI;
            OptInPeriod = 4;
            TypeOfPrice = PriceType.ClosePrice;
        }
        public CdlDoji(string symbol, DateOnly from, DateOnly to, int optInPeriod, PriceType price) : base(symbol, from, to, optInPeriod, price) 
        {
            summary.Indicator = IndicatorType.CDL_DOJI;
        }
        ...
        private void Calculate()
        {
            ...
            for (int i = 0; i < outNBElement; i++)
            {
                if (outInt[i] == 100 || outInt[i] == -100)
                    DOJI.Add(new Tuple<string, DateOnly>(Symbol, DateOnly.FromDateTime(equities[outBegIdx + i].DATE)), outInt[i]);
            }
        }
        private void IdentifyPatterns()
        {
            //1. For each Rsi captured
            foreach (var item in DOJI)
            {
