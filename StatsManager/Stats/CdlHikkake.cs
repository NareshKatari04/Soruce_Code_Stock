...
{
    public class CdlHikkake : Stat
    {
        //Static data
        public static Dictionary<Tuple<string, DateOnly>, double> HIKKAKE = new Dictionary<Tuple<string, DateOnly>, double>();

        //Constructors
        public CdlHikkake(string symbol) : base(symbol)
        {
            summary.Indicator = IndicatorType.CDL_HIKKAKE;
            OptInPeriod = 4;
            TypeOfPrice = PriceType.ClosePrice;
        }
        public CdlHikkake(string symbol, DateOnly from, DateOnly to, int optInPeriod, PriceType price) : base(symbol, from, to, optInPeriod, price) 
        {
            summary.Indicator = IndicatorType.CDL_HIKKAKE;
        }
        ...
        private void Calculate()
        {
            ...
            Core.CdlHikkake(begIdx, endIdx, inRealOpen.ToArray(), inRealHigh.ToArray(), inRealLow.ToArray(), inRealClose.ToArray(), out outBegIdx, out outNBElement, outInt);

            for (int i = 0; i < outNBElement; i++)
            {
                if (outInt[i] == 100 || outInt[i] == -100)
                    HIKKAKE.Add(new Tuple<string, DateOnly>(Symbol, DateOnly.FromDateTime(equities[outBegIdx + i].DATE)), outInt[i]);
            }
        }
        private void IdentifyPatterns()
        {
            //1. For each Rsi captured
            foreach (var item in HIKKAKE)
            {
