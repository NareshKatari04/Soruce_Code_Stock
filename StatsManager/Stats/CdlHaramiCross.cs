...
{
    public class CdlHaramiCross : Stat
    {
        //Static data
        public static Dictionary<Tuple<string, DateOnly>, double> HARAMI_CROSS = new Dictionary<Tuple<string, DateOnly>, double>();

        //Constructors
        public CdlHaramiCross(string symbol) : base(symbol)
        {
            summary.Indicator = IndicatorType.CDL_HARAMI_CROSS;
            OptInPeriod = 4;
            TypeOfPrice = PriceType.ClosePrice;
        }
        public CdlHaramiCross(string symbol, DateOnly from, DateOnly to, int optInPeriod, PriceType price) : base(symbol, from, to, optInPeriod, price) 
        {
            summary.Indicator = IndicatorType.CDL_HARAMI_CROSS;
        }
        ...
        private void Calculate()
        {
            ...
            Core.CdlHaramiCross(begIdx, endIdx, inRealOpen.ToArray(), inRealHigh.ToArray(), inRealLow.ToArray(), inRealClose.ToArray(), out outBegIdx, out outNBElement, outInt);

            for (int i = 0; i < outNBElement; i++)
            {
                if (outInt[i] == 100 || outInt[i] == -100)
                    HARAMI_CROSS.Add(new Tuple<string, DateOnly>(Symbol, DateOnly.FromDateTime(equities[outBegIdx + i].DATE)), outInt[i]);
            }
        }
        private void IdentifyPatterns()
        {
            //1. For each Rsi captured
            foreach (var item in HARAMI_CROSS)
            {
