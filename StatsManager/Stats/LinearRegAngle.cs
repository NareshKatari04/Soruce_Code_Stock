...
{
    public class LinearRegAngle : Stat
    {
        public Dictionary<Tuple<string, DateOnly>, double> LINEARREG_ANGLE = new Dictionary<Tuple<string, DateOnly>, double>();
        public LinearRegAngle(string symbol) : base(symbol) { OptInPeriod = 20; } //Default value
        public LinearRegAngle(string symbol, DateOnly from, DateOnly to, int optInPeriod, PriceType price) : base(symbol, from, to, optInPeriod, price) { }
        public void Process()
        {
            Calculate();
            IdentifyPatterns();
        ...
        public void Calculate()
        {
            ...
            Core.LinearRegAngle(begIdx, endIdx, inReal.ToArray(), OptInPeriod, out outBegIdx, out outNBElement, outReal);

            for (int i = 0; i < outNBElement; i++)
            {
                //Console.WriteLine( "DEBUG: EMA {0} {1} {2}", Symbol, DateOnly.FromDateTime(equities[outBegIdx + i].DATE), outReal[i]);
                LINEARREG_ANGLE.Add(new Tuple<string, DateOnly>(Symbol, DateOnly.FromDateTime(equities[outBegIdx + i].DATE)), outReal[i]);
            }
        }
        private void IdentifyPatterns()
        {
            ...
            foreach (KeyValuePair<Tuple<string, DateOnly>, double> item in LINEARREG_ANGLE)
            {
        ...
        {
            summary.Indicator = IndicatorType.LINEARREG_ANGLE;
            base.UpdateSummary();
        }
    }
}
