...
{
    public class CdlShortLine : Stat
    {
        //Static data
        public static Dictionary<Tuple<string, DateOnly>, double> SHORT_LINE = new Dictionary<Tuple<string, DateOnly>, double>();

        //Constructors
        public CdlShortLine(string symbol) : base(symbol)
        {
            summary.Indicator = IndicatorType.CDL_SHORT_LINE;
            OptInPeriod = 4;
            TypeOfPrice = PriceType.ClosePrice;
        }
        public CdlShortLine(string symbol, DateOnly from, DateOnly to, int optInPeriod, PriceType price) : base(symbol, from, to, optInPeriod, price)
        {
            summary.Indicator = IndicatorType.CDL_SHORT_LINE;
        }
        ...
        private void Calculate()
        {
            ...
            Core.CdlShortLine(begIdx, endIdx, inRealOpen.ToArray(), inRealHigh.ToArray(), inRealLow.ToArray(), inRealClose.ToArray(), out outBegIdx, out outNBElement, outInt);

            for (int i = 0; i < outNBElement; i++)
            {
                if (outInt[i] == 100 || outInt[i] == -100)
                    SHORT_LINE.Add(new Tuple<string, DateOnly>(Symbol, DateOnly.FromDateTime(equities[outBegIdx + i].DATE)), outInt[i]);
            }
        }
        private void IdentifyPatterns()
        {
            //1. For each Rsi captured
            foreach (var item in SHORT_LINE)
            {
