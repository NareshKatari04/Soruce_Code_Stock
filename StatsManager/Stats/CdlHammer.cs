...
{
    public class CdlHammer : Stat
    {
        //Static data
        public static Dictionary<Tuple<string, DateOnly>, double> HAMMER = new Dictionary<Tuple<string, DateOnly>, double>();

        //Constructors
        public CdlHammer(string symbol) : base(symbol)
        {
            summary.Indicator = IndicatorType.CDL_HAMMER;
            OptInPeriod = 4;
            TypeOfPrice = PriceType.ClosePrice;
        }
        public CdlHammer(string symbol, DateOnly from, DateOnly to, int optInPeriod, PriceType price) : base(symbol, from, to, optInPeriod, price) 
        {
            summary.Indicator = IndicatorType.CDL_HAMMER;
        }
        ...
        private void Calculate()
        {
            ...
            Core.CdlHammer(begIdx, endIdx, inRealOpen.ToArray(), inRealHigh.ToArray(), inRealLow.ToArray(), inRealClose.ToArray(), out outBegIdx, out outNBElement, outInt);

            for (int i = 0; i < outNBElement; i++)
            {
                if (outInt[i] == 100 || outInt[i] == -100)
                {
                    HAMMER.Add(new Tuple<string, DateOnly>(Symbol, DateOnly.FromDateTime(equities[outBegIdx + i].DATE)), outInt[i]);
                }
            }
        }
