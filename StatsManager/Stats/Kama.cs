...
{
    public class Kama : Stat
    {
        public Dictionary<Tuple<string, DateOnly>, double> KAMA = new Dictionary<Tuple<string, DateOnly>, double>();
        public Kama(string symbol) : base(symbol) { summary.Indicator = IndicatorType.KAMA; ; OptInPeriod = 20; } //Default value
        public Kama(string symbol, DateOnly from, DateOnly to, int optInPeriod, PriceType price) : base(symbol, from, to, optInPeriod, price) { summary.Indicator = IndicatorType.KAMA; }
        public void Process()
        {
            Calculate();
            IdentifyPatterns();
        ...
        public void Calculate()
        {
            ...
            for (int i = 0; i < outNBElement; i++)
            {
                ...
                KAMA.Add(new Tuple<string, DateOnly>(Symbol, DateOnly.FromDateTime(equities[outBegIdx + i].DATE)), outReal[i]);
            }
        }
        private void IdentifyPatterns()
        {
            ...
            foreach (KeyValuePair<Tuple<string, DateOnly>, double> item in KAMA)
            {
