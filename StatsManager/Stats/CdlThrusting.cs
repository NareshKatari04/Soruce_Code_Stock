...
{
    public class CdlThrusting : Stat
    {
        //Static data
        public static Dictionary<Tuple<string, DateOnly>, double> THRUST = new Dictionary<Tuple<string, DateOnly>, double>();

        //Constructors
        public CdlThrusting(string symbol) : base(symbol)
        {
            summary.Indicator = IndicatorType.CDL_THRUST;
            OptInPeriod = 4;
            TypeOfPrice = PriceType.ClosePrice;
        }
        public CdlThrusting(string symbol, DateOnly from, DateOnly to, int optInPeriod, PriceType price) : base(symbol, from, to, optInPeriod, price)
        {
            summary.Indicator = IndicatorType.CDL_THRUST;
        }
        ...
        private void Calculate()
        {
            ...
            Core.CdlThrusting(begIdx, endIdx, inRealOpen.ToArray(), inRealHigh.ToArray(), inRealLow.ToArray(), inRealClose.ToArray(), out outBegIdx, out outNBElement, outInt);

            for (int i = 0; i < outNBElement; i++)
            {
                if (outInt[i] == 100 || outInt[i] == -100)
                    THRUST.Add(new Tuple<string, DateOnly>(Symbol, DateOnly.FromDateTime(equities[outBegIdx + i].DATE)), outInt[i]);
            }
        }
        private void IdentifyPatterns()
        {
            //1. For each Rsi captured
            foreach (var item in THRUST)
            {
