...
{
    public class CdlMatchingLow : Stat
    {
        //Static data
        public static Dictionary<Tuple<string, DateOnly>, double> MATCHING_LOW = new Dictionary<Tuple<string, DateOnly>, double>();

        //Constructors
        public CdlMatchingLow(string symbol) : base(symbol)
        {
            summary.Indicator = IndicatorType.CDL_MATCHING_LOW;
            OptInPeriod = 4;
            TypeOfPrice = PriceType.ClosePrice;
        }
        public CdlMatchingLow(string symbol, DateOnly from, DateOnly to, int optInPeriod, PriceType price) : base(symbol, from, to, optInPeriod, price) 
        {
            summary.Indicator = IndicatorType.CDL_MATCHING_LOW;
        }
        ...
        private void Calculate()
        {
            ...
            Core.CdlMatchingLow(begIdx, endIdx, inRealOpen.ToArray(), inRealHigh.ToArray(), inRealLow.ToArray(), inRealClose.ToArray(), out outBegIdx, out outNBElement, outInt);

            for (int i = 0; i < outNBElement; i++)
            {
                if (outInt[i] == 100 || outInt[i] == -100)
                    MATCHING_LOW.Add(new Tuple<string, DateOnly>(Symbol, DateOnly.FromDateTime(equities[outBegIdx + i].DATE)), outInt[i]);
            }
        }
        private void IdentifyPatterns()
        {
            //1. For each Rsi captured
            foreach (var item in MATCHING_LOW)
            {
