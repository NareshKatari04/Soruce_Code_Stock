...
{
    public class CdlMatHold : Stat
    {
        //Static data
        public static Dictionary<Tuple<string, DateOnly>, double> MAT_HOLD = new Dictionary<Tuple<string, DateOnly>, double>();

        private double OptInPenetration;

        //Constructors
        public CdlMatHold(string symbol) : base(symbol)
        {
            summary.Indicator = IndicatorType.CDL_MATHOLD;
            OptInPeriod = 4;
            OptInPenetration = (-4e+37);
            TypeOfPrice = PriceType.ClosePrice;
        }
        public CdlMatHold(string symbol, DateOnly from, DateOnly to, int optInPeriod, PriceType price) : base(symbol, from, to, optInPeriod, price)
        {
            summary.Indicator = IndicatorType.CDL_MATHOLD;
        }
        ...
        private void Calculate()
        {
            ...
            Core.CdlMatHold(begIdx, endIdx, inRealOpen.ToArray(), inRealHigh.ToArray(), inRealLow.ToArray(), inRealClose.ToArray(), OptInPenetration, out outBegIdx, out outNBElement, outInt);

            for (int i = 0; i < outNBElement; i++)
            {
                if (outInt[i] == 100 || outInt[i] == -100)
                    MAT_HOLD.Add(new Tuple<string, DateOnly>(Symbol, DateOnly.FromDateTime(equities[outBegIdx + i].DATE)), outInt[i]);
            }
        }
        private void IdentifyPatterns()
        {
            //1. For each Rsi captured
            foreach (var item in MAT_HOLD)
            {
