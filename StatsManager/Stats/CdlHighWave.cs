...
{
    public class CdlHighWave : Stat
    {
        //Static data
        public static Dictionary<Tuple<string, DateOnly>, double> HIGH_WAVE = new Dictionary<Tuple<string, DateOnly>, double>();

        //Constructors
        public CdlHighWave(string symbol) : base(symbol)
        {
            summary.Indicator = IndicatorType.CDL_HIGH_WAVE;
            OptInPeriod = 4;
            TypeOfPrice = PriceType.ClosePrice;
        }
        public CdlHighWave(string symbol, DateOnly from, DateOnly to, int optInPeriod, PriceType price) : base(symbol, from, to, optInPeriod, price) 
        {
            summary.Indicator = IndicatorType.CDL_HIGH_WAVE;
        }
        ...
        private void Calculate()
        {
            ...
            Core.CdlHignWave(begIdx, endIdx, inRealOpen.ToArray(), inRealHigh.ToArray(), inRealLow.ToArray(), inRealClose.ToArray(), out outBegIdx, out outNBElement, outInt);

            for (int i = 0; i < outNBElement; i++)
            {
                if (outInt[i] == 100 || outInt[i] == -100)
                    HIGH_WAVE.Add(new Tuple<string, DateOnly>(Symbol, DateOnly.FromDateTime(equities[outBegIdx + i].DATE)), outInt[i]);
            }
        }
        private void IdentifyPatterns()
        {
            //1. For each Rsi captured
            foreach (var item in HIGH_WAVE)
            {
