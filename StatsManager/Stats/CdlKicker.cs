...
{
    public class CdlKicker : Stat
    {
        //Static data
        public static Dictionary<Tuple<string, DateOnly>, double> KICKER = new Dictionary<Tuple<string, DateOnly>, double>();

        //Constructors
        public CdlKicker(string symbol) : base(symbol)
        {
            summary.Indicator = IndicatorType.CDL_KICKER;
            OptInPeriod = 4;
            TypeOfPrice = PriceType.ClosePrice;
        }
        public CdlKicker(string symbol, DateOnly from, DateOnly to, int optInPeriod, PriceType price) : base(symbol, from, to, optInPeriod, price) 
        {
            summary.Indicator = IndicatorType.CDL_KICKER;
        }
        ...
        private void Calculate()
        {
            ...
            Core.CdlKicking(begIdx, endIdx, inRealOpen.ToArray(), inRealHigh.ToArray(), inRealLow.ToArray(), inRealClose.ToArray(), out outBegIdx, out outNBElement, outInt);

            for (int i = 0; i < outNBElement; i++)
            {
                if (outInt[i] == 100 || outInt[i] == -100)
                    KICKER.Add(new Tuple<string, DateOnly>(Symbol, DateOnly.FromDateTime(equities[outBegIdx + i].DATE)), outInt[i]);
            }
        }
        private void IdentifyPatterns()
        {
            //1. For each Rsi captured
            foreach (var item in KICKER)
            {
