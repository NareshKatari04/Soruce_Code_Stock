...
{
    public class CdlTasukiGap : Stat
    {
        //Static data
        public static Dictionary<Tuple<string, DateOnly>, double> TASUKI_GAP = new Dictionary<Tuple<string, DateOnly>, double>();

        //Constructors
        public CdlTasukiGap(string symbol) : base(symbol)
        {
            summary.Indicator = IndicatorType.CDL_TASKI_GAP;
            OptInPeriod = 4;
            TypeOfPrice = PriceType.ClosePrice;
        }
        public CdlTasukiGap(string symbol, DateOnly from, DateOnly to, int optInPeriod, PriceType price) : base(symbol, from, to, optInPeriod, price)
        {
            summary.Indicator = IndicatorType.CDL_TASKI_GAP;
        }
        ...
        private void Calculate()
        {
            ...
            Core.CdlTasukiGap(begIdx, endIdx, inRealOpen.ToArray(), inRealHigh.ToArray(), inRealLow.ToArray(), inRealClose.ToArray(), out outBegIdx, out outNBElement, outInt);

            for (int i = 0; i < outNBElement; i++)
            {
                if (outInt[i] == 100 || outInt[i] == -100)
                    TASUKI_GAP.Add(new Tuple<string, DateOnly>(Symbol, DateOnly.FromDateTime(equities[outBegIdx + i].DATE)), outInt[i]);
            }
        }
        private void IdentifyPatterns()
        {
            //1. For each Rsi captured
            foreach (var item in TASUKI_GAP)
            {
