...
{
    public class Min : Stat
    {
        public Dictionary<Tuple<string, DateOnly>, double> MIN = new Dictionary<Tuple<string, DateOnly>, double>();
        public Min(string symbol) : base(symbol) { OptInPeriod = 90; } //Default value
        public Min(string symbol, DateOnly from, DateOnly to, int optInPeriod, PriceType price) : base(symbol, from, to, optInPeriod, price) { }
        public void Process()
        {
            Calculate();
            IdentifyPatterns();
        ...
        public void Calculate()
        {
            ...
            Core.Min(begIdx, endIdx, inReal.ToArray(), OptInPeriod, out outBegIdx, out outNBElement, outReal);

            for (int i = 0; i < outNBElement; i++)
            {
                //Console.WriteLine( "DEBUG: EMA {0} {1} {2}", Symbol, DateOnly.FromDateTime(equities[outBegIdx + i].DATE), outReal[i]);
                MIN.Add(new Tuple<string, DateOnly>(Symbol, DateOnly.FromDateTime(equities[outBegIdx + i].DATE)), outReal[i]);
            }
        }
        private void IdentifyPatterns()
        {
            //1. List of all EMA values
            foreach (KeyValuePair<Tuple<string, DateOnly>, double> item in MIN)
            {
        ...
        {
            summary.Indicator = IndicatorType.MIN;
            base.UpdateSummary();
        }
    }
}
